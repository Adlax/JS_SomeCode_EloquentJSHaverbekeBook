...
function parse(program){
  ...
  if(skipSpace(result.rest).length>0){throw new SyntaxError("Unexpected text after program");}
  return result.expr;
}
...
function evaluate(expr,env){
  switch(expr.type){
    ...
    case "word":
      ...
      else throw new referenceError("Undefined error in env: " + expr.name);
    case "apply":
...
specialForms["fun"] = function(args,env){
  ...
  return function(){
    ...
    for(var i=0;i<arguments.length;i++) localEnv[argNames[i]] = arguments[i];
