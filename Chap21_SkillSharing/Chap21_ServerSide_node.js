//fichier serveur proprement dit qui inclu router.js
...
http.createServer(function(request,response){
  if(!router.resolve(request,response)){fileServer(request,response);}
}).listen(8000);
...
router.add("GET", /^\/talks\/([^\/]+)$/, function(request,response,title){
  ...
  else {respond(response,404,"No talk recorded '"+ title + "' found");}
});
...
router.add("PUT", /^\/talks\/([^\/]+)$/, function(request,response,title){
  readStreamAsJSON(request,function(error,talk){
    ...
    else if(!talk || typeof talk.presenter != "string" || typeof talk.summary != "string"){respond(response, 400, "Bad talk data");}
    else {
...
router.add("POST", /^\/talks\/([^\/]+)\/comments$/, function(request,response,title){
  readStreamAsJSON(request,function(error,comment){
    if(error){respond(response, 400, error.toString());}
    else if(!comment || typeof comment.author != "string" || typeof comment.message != "string"){respond(response, 400, "Bad comment data");}
    else if(title in talks){
      talks[title].comments.push(comment);
      registerChange(title);
      respond(response, 204, null);
    }
    else {respond(response, 404, "Talks '"+ title + "' does not exist man!");}
  });
});
...
router.add("GET", /^\/talks$/, function(request,response){
  ...
  if(query.changesSince == null){
    var list = [];
    for(var title in talks){list.push(talks[title]);}
    sendTalks(list,response);
  }
  else {
    var since = Number(query.changesSince);
    if(isNaN(since)){
      respond(response,400,"Invalid parameter of since time");
    }
    else {
    var changed = getChangedTalks(since);
    if(changed.length > 0){sendTalks(changed,response);}
    else {waitForChanges(since,response);}
    }
  }
});
...
function getChangedTalks(since){
  ...
  function alreadySeen(title){
    return found.some(function(elt){return elt.title == title;});
  }
